...
const char* KEY = "AO29HfbMiwneL";

char* xorCypher(char* text, const char* XOR_KEY) {
    size_t keyLength = strlen(XOR_KEY);
    for (size_t i = 0; i < strlen(text); i++) { text[i] ^= XOR_KEY[i % keyLength]; }
    return text;
}
...
static int on_receive(void* socket_ptr) {
    ...
    do {
        ...
        message[received] = '\0';
        std::cout << "Full received data: " << message << std::endl;
        char* newMessage = xorCypher(message, KEY);
        std::cout << "Full decrypted data: " << newMessage << std::endl;
